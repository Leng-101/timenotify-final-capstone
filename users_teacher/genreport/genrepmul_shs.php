...
function Header()
{
    ...
    $this->Cell(100);
    $this->Cell(88,-4,'ADMIN REPORT - SENIOR HIGH SCHOOL DEPARTMENT' ,0,1,'C');
    // Line break
...
$pdf->Cell(270,10,'TEACHER - TIME RECORDS',0,1, 'C');

$pdf->Cell(190,5,' ',0,1, 'C');


//GET THE SELECTED VALUE IN DROPDOWN BUTTON~
$shs_grade = $_POST['gradeyear'];
$shs_strand = $_POST['strandList'];
$shs_sec = $_POST['sectionList'];
$shs_daterec = $_POST['dateList'];

$sql = "SELECT s.stud_id, s.fullname, s.sex, s.grade_year, s.section,
                s.strand, t.daterec, t.type, t.time, t.daterec 
        FROM students s INNER JOIN time_in_records t ON s.studnum = t.studnum
        WHERE s.grade_year = '$shs_grade' AND s.section = '$shs_sec' 
        AND t.daterec = '$shs_daterec'AND s.strand='$shs_strand' ORDER BY lastname";

$res = mysqli_query($con,$sql);
$count = mysqli_num_rows($res);

$pdf->Cell(12,10,'Date: ',0,0); 
$pdf->Cell(210,10,$shs_daterec,0,1);

$pdf->Cell(39,10,'Number of Records: ',0,0); 
$pdf->Cell(30,10,$count,0,0); 

$pdf->Cell(16,10,'Strand: ',0,0); 
$pdf->Cell(25,10,$shs_strand,0,0);

$pdf->Cell(26,10,'Grade Level: ',0,0); 
$pdf->Cell(30,10,$shs_grade,0,0);

$pdf->Cell(18,10,'Section: ',0,0); 
$pdf->Cell(15,10,$shs_sec,0,1);
...
$sqlSHS = "SELECT s.stud_id, s.fullname, s.sex, s.grade_year, s.section,
                s.strand, t.daterec, t.type, t.time, t.daterec 
        FROM students s INNER JOIN time_in_records t ON s.studnum = t.studnum
        WHERE s.grade_year = '$shs_grade' AND s.section = '$shs_sec' 
        AND t.daterec = '$shs_daterec'AND s.strand='$shs_strand' ORDER BY lastname";

$result=mysqli_query($con,$sqlSHS);
