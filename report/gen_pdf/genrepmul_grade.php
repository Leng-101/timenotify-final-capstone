...
function Header()
{
    ...
    $this->Cell(100);
    $this->Cell(68,-4,'ADMIN REPORT - ELEMENTARY DEPARTMENT',0,1,'C');
    // Line break
...
$grschool_grade = $_POST['gradeyear'];
$grschool_sec = $_POST['sectionList'];
$grschool_daterec = $_POST['dateList'];
...
$sql = "SELECT s.stud_id, s.fullname, s.sex, s.grade_year, s.section,
        ...
        WHERE s.grade_year = '$grschool_grade' AND s.section = '$grschool_sec' 
        AND t.daterec = '$grschool_daterec' ORDER BY lastname";
...
$pdf->Cell(210,10,$grschool_daterec,0,1);
...
$pdf->Cell(30,10,$grschool_grade,0,0);

$pdf->Cell(18,10,'Section: ',0,0); 
$pdf->Cell(15,10,$grschool_sec,0,1);
...
$sqldisplay = "SELECT s.stud_id, s.fullname, s.sex, s.grade_year, s.section,
                t.daterec, t.type, t.time, t.daterec 
                FROM students s INNER JOIN time_in_records t ON s.studnum = t.studnum
                WHERE s.grade_year = '$grschool_grade' AND s.section = '$grschool_sec' AND t.daterec = '$grschool_daterec'";
